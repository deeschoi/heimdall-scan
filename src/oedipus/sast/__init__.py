"""SAST: run Semgrep's custom Oedipus rules and emit the shared Finding schema.

This is the static half of the "find -> prove -> patch" story: the same
``Finding`` model, reporters, and ``oedipus gate`` filters used for DAST work
unmodified on SAST hits. :mod:`oedipus.correlate` joins the two on
...
from oedipus.sast.runner import SastError, run_semgrep
...
# check -- this is also what makes `oedipus correlate` line up cleanly.
...
def _finding_from_result(result: dict[str, Any], index) -> Finding:
    ...
    severity_name = metadata.get("oedipus_severity")
    severity = (
